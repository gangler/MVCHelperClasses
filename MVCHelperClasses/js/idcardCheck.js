...
function IsIdcard(value) {
    var b = isCardNo(value) && checkProvince(value) && checkBirthday(value) && checkParity(value);
    return b;
}
function checkID(value) {
   
    if (!(/^\d{15}$|^\d{18}$|^\d{17}[xX]$/.test(value))) {//验证没通过
        return false;
    } else
    {
        return true;
